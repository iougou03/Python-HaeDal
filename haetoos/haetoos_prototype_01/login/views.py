...
def sign_in(request):
    if request.method == 'GET':
        ...
        try:
            person = list(HaedalMember.objects.filter(haetoos_id=haetoos_id))
        except HaedalMember.DoesNotExist:
            message = 'account doesn\'t exist'
            return render(request,'login/sign_in.html', {'message':message})
        for p in person:
            if check_password(haetoos_ps,p.haetoos_ps):
                message = 'logined Succesfully!'
                request.session['user']=p.haetoos_id
                status=1
                break
            status=0
        if status==0:
...
def sign_up(request) :
    if request.method == 'GET':
        ...
        if not haetoos_id or not haetoos_ps:
            res['message'] = 'please type id or password'
            ...
            try: 
                person = HaedalMember.objects.get(student_id = student_id)
            except: #학번이 등록되어 있지 않은 경우
                res['message'] = 'Failed to Create. Ask manager to join the club'
                return render(request,'login/result.html',res)
